...
const ImageSpawner: React.FC<ImageSpawnerProps> = ({ children, enableSpawning = false }) => {
  ...
  return (
    <div 
      ...
      onClick={enableSpawning ? spawnImage : undefined}
    >
      {children}
      
      {enableSpawning && spawnedImages.length > 0 && (
        <button
          onClick={(e) => {
            e.stopPropagation();
            resetImages();
